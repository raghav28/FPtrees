...
using namespace std;
float minconf=0.7;          //minconf
ofstream ft,fo;
struct node         //structutre of nodes in FPtree
{
	map<int,struct node*> m;        //map values point to child nodes
	int counter;                    //counter
	int value;                      //value represents item
	int flag;			//flag to represent if node is part of conditional fptree
	int pole;	//instead of removing nodes from conditional fptree with itemCount<thresh_count we set pole=1 and ignore them.
	struct node *parent,*l;		//pointers to parent and (same_items in other branches) 
};
struct tree		//tree datastructure has nodes
{
	node* nlinks[60];	//nlinks[i] is head of linked list which connects items i.
	node *troot;		//root of tree
};

struct freq_item_set
{
    vector<int> v;		// vector of items in itemset.
    int cnt;			//total count of the item_set
};

vector<struct freq_item_set> arr[10];	//arr[i] has frequent_itemsets of size i.
...
float data[768][9];
int it_count[50];		//count of item->i in it_count[i]
void encode()
{
	std::ifstream file("diabities");	// file is file pointer.
	string line;
	int j,i;
	for(j=0;j<768;j++)
	{
	    int count=0;
	 	std::getline(file,line);	//reading data from file line by line.
...

		{
			...
			char *a = const_cast<char*>(item.c_str());	//getting a feature each time by looping.',' as delimiter.
            if(i==0 || i==8)
                data[j][i]=float(atoi(a));
            else
                data[j][i]=float(atof(a));		//data in string format to float/int.
            if(i!=0 && i!=8 && (data[j][i]==0.0 || data[j][i]==0))
            {
                data[j][i]=-500.0; //except for first column and last column data,other 0 data represents missing data,so making it -500. 
            }
        }
	}
}
...
node* copyfnc(node* actroot,node* root,node* nlinks[])	//takes tree root and replicates and returns the root of the replicated tree.
{

	node *temp = root;
	node *nroot = new node();		//nroot is the pointer to root of new tree.
	...
	{
		nroot->m[it->first] = copyfnc(nroot,temp->m[it->first],nlinks);//calling copyfnc() recursively on child nodes.
	}
	return nroot;
}
int traverse(node* root,int item_count[],int k)
{
    ...
    if(temp->value==k)
    {
        temp->m.clear();  //in process of building conditional fptreeending with k, delete children of k.
        return temp->counter; //base case of recursive call where u return counter value of the item.
    }
    map<int,struct node*>::iterator it;
    temp->counter=0;
    for(it=temp->m.begin();it!=temp->m.end();it++)
    {
        ...
        {
            temp->counter+= traverse(temp->m[it->first],item_count,k);	//updating counter values.
            item_count[temp->value] += temp->counter; 	//calculating counter values.
        }
    }
...
void traverse1(node *root,int item_count[])
{
    ...
    for(it=root->m.begin();it!=root->m.end();it++)
    {
        ...
        if(item_count[val]<thresh_count)
            root->m[it->first]->pole = -1;  		//setting pole=-1 for nodes whose itemcount<threshcount.
...
void make_sets(node *root,list<pii> l)
{
    ...
    {
        titem.v.push_back(it1->second);		//building frequent item sets.
    }
...
int main()
{
    ...
    ft.open("out1.txt");
	int tot_items = 45;
	preprocess();	//call to preprocess.
	tree t ;	//  actual fptree.
	for(int i=0;i<60;i++)
		t.nlinks[i]=NULL;
	t.troot = new node();
	t.troot->value = 0;	//null node in fptree, represented by making value=0.
	map<int,struct node*>::iterator it;
	for(int i=0;i<768;i++)	//Constructing actual fptree, by considering transaction by transaction.
	{
		node *temp = t.troot;
		sort(data1[i].begin(),data1[i].end());
		for(int j=0;j<data1[i].size();j++)
		{
			if(it_count[data1[i][j]]>thresh_count)	//enter nodes into fptree only if wholeCount of item>thresholdCount
			{
                int k = data1[i][j];
                it = temp->m.find(k);	 
                if(it==temp->m.end()) //At a given fptree level check for item, if not present add it to fptree(i.e.make an entry into map)
                {
                    temp->m[k] = new node();
                    ...
                    temp->m[k]->pole=0;
                    temp->l = t.nlinks[k];	 
                    t.nlinks[k] = temp;	//entering new nodes address into nlinks i.e., entering latest item i address in nlinks[i].
                }
                else
                {
                    temp->m[k]->counter++;
	...
	for(int i = tot_items;i>0;i--)
	{
		if(it_count[i]>thresh_count)
		{
			...
			make_sets(t.troot,l1);		//making call to make_sets where frequent itemsets actually generated.
			l1.pop_front();
		}
	}
	for(int i=2;i<10;i++)
